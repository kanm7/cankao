...
*/
#define _GNU_SOURCE  /* 必须在最前面，解决 CPU_ZERO 和 memmem 的警告 */
#include <fcntl.h>
#define AFL_MAIN
...
#define FLIP_BIT(_ar, _b) do { \
  ...
  // 我们小组加的调试代码
{
  FILE* f_log = fopen("mutation_debug.log", "a");
  if (f_log) {
    fprintf(f_log, "Entering Stage: %s | Current Seed: %s\n", stage_name, queue_cur->fname);
    fclose(f_log);
  }
}
